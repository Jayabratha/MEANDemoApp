...
exports.insertUser = function(res, username, sex, dob, address, role, salary, email, password) {
	...
	user.save(function(err) {
		if(err){
			...
			if(duperror){
				res.send({success: false, message: "Sorry! We already have a registered user with that username/email"});
			}
			else {
				res.send({success: false, message: "Sorry! We couldn't create your profile. Please try later"});
			}
			...
			res.send({success: true, message: "We have succesfully created your profile"});
		}
	})
...
exports.authUser = function(res, email, password){
	...
	User.find({ "email" : email }, function(err, users) {
		if(err){
			...
			res.send({success: false, message: "Sorry! Couldn't find " + email + " in our DB"});
		}
		else {
			...
			if(users.length > 0){
				...
				if(firstuser.password == password){
					...
					res.cookie('password', firstuser.password);
					res.send({success: true, message: "User found in DB"});
				}
				else {
				...
				res.send({success: false, message: "No user found. Please register first"});
			}
...
exports.getUser = function(res, username, password) {
	User.find({ "username" : username }, function(err, users) {
		if(err){
			...
			res.send({success: false, message: "Sorry! Couldn't find " + username + " in our DB"});
		}
		else {
			console.log(users.length + " User found");
			for(var user in users) {
			...
			if(firstuser.password == password){
				console.log(firstuser);
				res.send({success: true, message: "User found in DB", userObj: firstuser});
			}
			else {
				console.log("WRONG PASSWORD!");
				res.send({success: false, message: "Wrong Password! Please try again"});
